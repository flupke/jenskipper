...
def import_(jenkins_url, dest_dir):
    ...
    with click.progressbar(jobs, label='Importing jobs') as bar:
...
def _write_jobs_templates(jobs, dest_dir, jenkins_url):
    pipes_bits = {}
    jobs_templates = {}
    for job_name in jobs:
        ...
        tpl_fname = job_config.get_default_template_fname(dest_dir, job_name)
...
def _write_jobs_defs(jobs_templates, dest_dir):
    fname = job_config.get_jobs_defs_fname(dest_dir)
    text = job_config.format_default_jobs_defs(jobs_templates, dest_dir)
...
def _write_default_context(dest_dir):
    fname = job_config.get_default_context_fname(dest_dir)
    open(fname, 'w').close()
